...
function GreetingHeader({ dark, onToggleTheme }) {
  return (
    <header className="bk-row" style={{ gap: 12, padding: '14px 0 18px', alignItems: 'flex-start' }}>
      ...
      <button className="bk-iconbtn" aria-label="Сменить тему" onClick={onToggleTheme} style={{ marginTop: 14 }}>
...
function BookHeroCard({ book, onMark, onEdit, onQuick, nudge, marked, todayDelta, comeback }) {
      ...
      {marked ? (
        <>
          <div className="bk-row" style={{ gap: 10 }}>
            <div style={{
              width: 30, height: 30, borderRadius: '50%', background: 'var(--sage-soft)',
              display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--sage)', flex: 'none',
            }}>
              <BkIcon name="check" size={15} stroke={2.6} />
            </div>
            <span style={{ fontSize: 14.5, fontWeight: 600 }}>
              Отмечено сегодня{todayDelta > 0 ? ' · +' + todayDelta + ' стр.' : ''}
            </span>
          </div>
          <button className="bk-btn bk-btn-ghost" onClick={() => onMark(book)}>Добавить ещё страницы</button>
        </>
      ) : (
        <>
          <div className="bk-quick" data-comment-anchor="quick-log">
            {[10, 25, 50].map((n) => (
              <button key={n} aria-label={'Добавить ' + n + ' страниц'} onClick={() => onQuick(book, n)}>+{n}</button>
            ))}
            <button aria-label="Точный ввод" onClick={() => onMark(book)}><BkIcon name="plus" size={18} stroke={2.2} /></button>
          </div>
          <button className="bk-btn bk-btn-primary" onClick={() => onMark(book)}>
            {comeback ? 'Продолжить чтение' : 'Отметить прогресс'}
          </button>
        </>
      )}
    </section>
...
function HomeScreen({ app }) {
  const { books, streak, todayMarked, nudge, comeback, goal, todayDelta } = app.state;
  ...
  return (
    <div className="bk-screen" data-screen-label="Активность">
      ...
      <StreakBlock streak={streak} todayMarked={todayMarked} comeback={comeback} onBell={app.openReminder} />
      {books.length > 0 && <div style={{ marginTop: 12 }}><GoalBar goal={goal} variant={app.goalStyle} /></div>}
      ...
      {active && (
        <>
          <BookHeroCard book={active}
            nudge={nudge && !todayMarked}
            marked={todayMarked}
            todayDelta={todayDelta}
            comeback={comeback}
            onMark={app.openProgress}
            onEdit={app.openEdit}
            onQuick={app.quickLog} />
          {(others.length > 0 || planned.length > 0) && (
            <section style={{ marginTop: 24 }}>
              <div className="bk-label" style={{ marginBottom: 12 }}>Ещё в чтении и планах</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {others.map((b) => <BookListItem key={b.id} book={b} onClick={() => app.openBook(b)} />)}
                {planned.slice(0, 2).map((b) => <BookListItem key={b.id} book={b} onClick={() => app.openBook(b)} />)}
              </div>
            </section>
          )}
        </>
...
function StatsScreen({ app }) {
  ...
  const { hasHistory, streak, goal, record } = app.state;
  const D = BK_DATA;
    ...
    <div className="bk-screen" data-screen-label="Продуктивность">
      ...
      {hasHistory && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginTop: 12 }}>
          <GoalBar goal={goal} onClick={app.openGoal} variant={app.goalStyle} />
          <div className="bk-insight" data-comment-anchor="insight">
...
function LibraryScreen({ app }) {
  ...
  return (
    <div className="bk-screen" data-screen-label="Библиотека" style={{ paddingLeft: 0, paddingRight: 0 }}>
      ...
      <button className="bk-fab" aria-label="Добавить книгу" onClick={() => app.openSearch()}>
...
function LibraryCard({ book, app }) {
  const pct = book.pages ? (book.current / book.pages) * 100 : 0;
  return (
    <div className="bk-gridcard" onClick={() => app.openBook(book)}>
      <div style={{ position: 'relative' }}>
