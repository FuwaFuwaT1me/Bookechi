...
(function () {
  ...
  function makeBooks() {
    _id = 0;
    return [
      book('Норвежский лес', 'Харуки Мураками', 320, 'reading', { current: 182, fav: true, active: true }),
      book('Думай медленно… решай быстро', 'Даниэль Канеман', 456, 'reading', { current: 120 }),
  ...
  const SCENARIOS = {
    'Наполненный':         () => ({ books: makeBooks(), streak: 7, todayMarked: false, hasHistory: true, nudge: false }),
    'Без отметки сегодня': () => ({ books: makeBooks(), streak: 7, todayMarked: false, hasHistory: true, nudge: true }),
    'Нет активной книги':  () => {
      const books = makeBooks().map(b => b.status === 'reading' ? { ...b, status: 'planned', current: 0, active: false } : b);
      return { books, streak: 3, todayMarked: false, hasHistory: true, nudge: false };
    },
    'Пустая библиотека':   () => ({ books: [], streak: 0, todayMarked: false, hasHistory: false, nudge: false }),
  };
  ...
  window.BK_DATA = {
    SCENARIOS, TODAY, MONTHS_RU, MONTHS_RU_SHORT, WEEKDAYS_RU, COVER_TONES,
    monthGrid, monthSummary, yearGrid, yearSummary, pagesOn, levelFor, greeting,
  };
})();
